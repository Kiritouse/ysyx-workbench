...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (s->pc+imm)&~3ull;R(rd)=s->snpc); //添加了地址的4字节对齐
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (imm+src1)&~3ull;R(rd)=s->snpc);//添加了跳转地址的4字节对齐
  //注意这条指令要对齐到4的倍数，所以要把最后一位清零
//add-longlong.c
...
//bubble-sort.c
  //INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B,if((int32_t)src1>=(int32_t)src2) s->dnpc = s->pc + imm);为什么这里加上(int32_t)就不会报错？
...
//剩下的一些指令
/*ps:之前这里的slt是这样写的,  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = src1 < src2? 1: 0); 结果就报错了*/
...
//end of your code
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0,used as nemu_trap
